//
//  ListPendingOrderTableViewCell.swift
//  SEEMT
//
...
class ListPendingOrderTableViewCell: UITableViewCell {
    ...
    // MARK: - Variable -
    public var data: SupplierOrdersRequest? = nil {
        didSet {
            
            lbl_create_at.text = data?.created_at
            lbl_code.text = String(data?.id ?? 0)
            lbl_total_amount.text = Utils.stringVietnameseMoneyFormatWithNumber(amount: data?.total_amount ?? 0)
            
            lbl_quantity.text = Utils.stringVietnameseMoneyFormatWithNumber(amount: data?.quantity ?? 0)
