//
//  DialogChooseInputViewController.swift
//  TECHRES-SUPPLIER
//
...
class DialogChooseInputViewController: UIViewController {
    ...
    @IBAction func actionInputPercent(_ sender: Any) {
        self.presentModalInputQuantityViewController(number: viewModel.discount_percent.value)
    }
    ...
    @IBAction func actionApproved(_ sender: Any) {
        if viewModel.discount_amount.value == 0{
            self.delegate?.callBackChooseInputMoney(amount: viewModel.discount_amount_display.value, percent: viewModel.discount_percent.value)
        }else{
            self.delegate?.callBackChooseInputMoney(amount: viewModel.discount_amount.value, percent: viewModel.discount_percent.value)
        }
        dismiss(animated: true)
    }
