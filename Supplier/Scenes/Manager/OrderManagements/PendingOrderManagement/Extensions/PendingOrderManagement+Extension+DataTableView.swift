//
//  PendingOrderManagementViewController+Extension+DataTableView.swift
//  Techres-Seemt
//
...
extension PendingOrderManagementViewController{
    func registerCell() {
        ...
        let pendingOrderTableViewCell = UINib(nibName: "PendingOrderTableViewCell", bundle: .main)
        tableView.register(pendingOrderTableViewCell, forCellReuseIdentifier: "PendingOrderTableViewCell")
        ...
        tableView.rx.modelSelected(Restaurant.self).subscribe(onNext: { [self] element in
            ...
            self.viewModel.makeListPendingOrderViewController(restaurant_id: element.id)
        }).disposed(by: rxbag)
    ...
    func bindTableViewData() {
        viewModel.dataArray.bind(to: tableView.rx.items(cellIdentifier: "PendingOrderTableViewCell", cellType: PendingOrderTableViewCell.self))
            {  (row, data, cell) in
                cell.data = data
