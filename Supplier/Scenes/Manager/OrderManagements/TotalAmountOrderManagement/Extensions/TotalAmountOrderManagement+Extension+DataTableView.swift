//
//  TotalAmountOrderManagement+Extension+DataTableView.swift
//  TECHRES-SUPPLIER
//
...
extension TotalAmountOrderManagementViewController {
    ...
    func registerCell() {
        let totalAmountOrderTableViewCell = UINib(nibName: "TotalAmountOrderTableViewCell", bundle: .main)
        tableView.register(totalAmountOrderTableViewCell, forCellReuseIdentifier: "TotalAmountOrderTableViewCell")
    ...
    func bindTableViewData() {
        viewModel.dataArray.bind(to: tableView.rx.items(cellIdentifier: "TotalAmountOrderTableViewCell", cellType: TotalAmountOrderTableViewCell.self))
            {  (row, data, cell) in
                ...
                cell.btnCheck.rx.tap.asDriver()
                   .drive(onNext: { [self] in
                       ...
                       var cloneMaterialSelected = self.material_selected
                       ...
                       let newTotalAmount = selectedItems.reduce(0) { $0 + $1.total_amount_from_quantity_import }
                       lbl_total_amount.text = Utils.stringVietnameseMoneyFormatWithNumber(amount: newTotalAmount)
