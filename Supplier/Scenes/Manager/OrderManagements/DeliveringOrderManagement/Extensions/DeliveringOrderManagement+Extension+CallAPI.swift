//
//  DeliveringOrderManagementViewController+Extension.swift
//  Techres-Seemt
//
...
extension DeliveringOrderManagementViewController {
    func getSupplierOrdersGroupsList(){
        viewModel.getlistRestaurant().subscribe(onNext: { [self] (response) in
            if(response.code == RRHTTPStatusCode.ok.rawValue){
                if let dataFromServer = Mapper<RestaurantModel>().map(JSONObject: response.data) {
                    ...
                    viewModel.dataArray.accept(cloneData.filter({$0.total_delivering > 0}))
