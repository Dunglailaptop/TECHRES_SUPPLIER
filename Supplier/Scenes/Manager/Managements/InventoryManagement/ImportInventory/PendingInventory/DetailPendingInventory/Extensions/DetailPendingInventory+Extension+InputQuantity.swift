...
extension DetailPendingInventoryViewController: InputQuantityDelegate{
    ...
    func callbackInputQuantity(number: Float, position: Int) {
        ...
        if viewModel.type_select_input_quantity.value == 1 {
            ...
            self.viewModel.vat_percent.accept(Int(number))
            ...
            cloneDataArray[position].supplier_input_quantity = Double(number)
