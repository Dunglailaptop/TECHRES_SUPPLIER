...
extension ReturnItemsReportViewController{
    func setUpPieChart(dataChart: [MaterialReportData]) {
        ...
        for i in 0 ..< dataChart.count {
            s += Double(dataChart[i].total_return)
        }
        
        for i in 0 ..< dataChart.count {
            pieChartItems.append(PieChartDataEntry(value: Double(dataChart[i].total_return), label: Utils.stringVietnameseMoneyFormatWithNumber(amount: dataChart[i].total_return)))
        }
