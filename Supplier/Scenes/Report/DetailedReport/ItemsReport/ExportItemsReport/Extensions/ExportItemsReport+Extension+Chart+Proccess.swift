...
extension ExportItemsReportViewController{
    func setUpPieChart(dataChart: [MaterialReportData]) {
        ...
        for i in 0 ..< dataChart.count {
            s += Double(dataChart[i].total_export)
        }
        
        for i in 0 ..< dataChart.count {
            pieChartItems.append(PieChartDataEntry(value: Double(dataChart[i].total_export), label: Utils.stringVietnameseMoneyFormatWithNumber(amount: dataChart[i].total_export)))
        }
