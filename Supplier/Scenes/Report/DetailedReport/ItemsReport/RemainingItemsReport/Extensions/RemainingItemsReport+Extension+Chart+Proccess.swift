//
//  RemainingItemsReportViewController+Extension+Chart+Proccess.swift
//  Techres-Seemt
//
...
extension RemainingItemsReportViewController{
    func setUpPieChart(dataChart: [MaterialReportData]) {
        ...
        for i in 0 ..< dataChart.count {
            s += Double(dataChart[i].total_remaining)
        }
        
        for i in 0 ..< dataChart.count {
            pieChartItems.append(PieChartDataEntry(value: Double(dataChart[i].total_remaining), label: Utils.stringVietnameseMoneyFormatWithNumber(amount: dataChart[i].total_remaining)))
        }
