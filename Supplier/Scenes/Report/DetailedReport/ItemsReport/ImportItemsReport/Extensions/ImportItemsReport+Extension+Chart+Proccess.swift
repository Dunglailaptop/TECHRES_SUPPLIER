...
extension ImportItemsReportViewController{
    func setUpPieChart(dataChart: [MaterialReportData]) {
        ...
        for i in 0 ..< dataChart.count {
            s += Double(dataChart[i].total_import)
        }
        
        for i in 0 ..< dataChart.count {
            pieChartItems.append(PieChartDataEntry(value: Double(dataChart[i].total_import), label: Utils.stringVietnameseMoneyFormatWithNumber(amount: dataChart[i].total_import)))
        }
