...
class BaseViewModel {
    ...
    let successHandler: ((AFDataResponse<Any>) -> Void)? = {res in
        //        self.getAvatarURL(fileName: filename,width: width,height: height)
        dLog(res)
    }
...
//    http://172.16.10.85:9007/api/v1/media/generate
        self.uploadWithData(serverUrl:  URL(string: String(format: "%@%@",APIEndPoint.endPointURL, "/api/v2/media/upload/"))!, headers: headers, progressing: progressHandler, success: successHandler, failure: errorHandler)
    }
