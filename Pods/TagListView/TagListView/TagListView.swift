...
open class TagListView: UIView {
    ...
    func createNewTagView(_ title: String,id:Int = 0) -> TagView {
    ...
    @discardableResult
    open func addTag(_ title: String,id:Int = 0) -> TagView {
        defer { rearrangeViews() }
        return addTagView(createNewTagView(title,id: id))
    }
    
    @discardableResult
    open func addTags(_ titles: [(title:String, id:Int)]) -> [TagView] {
        //Pham khanh huy added this mapping computed properties
        let mapping:(String,Int) -> TagView = {(title, id) -> TagView in self.createNewTagView(title,id:id)}
        return addTagViews(titles.map(mapping))
    }
